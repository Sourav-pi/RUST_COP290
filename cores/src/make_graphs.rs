...
impl Sheet {
    ...
    pub fn pie_graph(
        ...
        if start.0==end.0 {
            for i in start.1..=end.1 {
                values.push(self.grid[start.0][i].value.clone());
                if x_labels.len()<=cnt{
            ...
            for i in start.0..=end.0 {
                values.push(self.grid[i][start.1].value.clone());
                if x_labels.len()<=cnt{
                    x_labels.push(format!("{}",cnt+1));
                }
                cnt+=1;
    ...
    let data: Vec<(i32,String)> = values.into_iter()
    .zip(x_labels.into_iter()).collect();
    println!("{:?}",data);
        Ok(Chart::new()
    ...
    pub fn scatter_graph(
        ...
        for i in 0..diff1+1 {
            for j in 0..diff2+1 {
                ...
                temp_vec.push(self.grid[start1.0+i as usize][start1.1+j as usize].value.clone());
